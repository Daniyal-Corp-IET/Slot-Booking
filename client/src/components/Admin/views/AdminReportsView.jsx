    ...
    return (
        <div className="mx-auto max-w-400 space-y-5">
            <div className="flex justify-end">
                <AdminAction
                    className="inline-flex items-center gap-2 rounded-xl border border-itx-border bg-white px-4 py-2.5 text-xs font-bold text-itx-ink shadow-sm transition hover:border-[#128a93]/40 hover:text-[#128a93] hover:shadow-[0_12px_25px_-18px_rgba(18,138,147,0.5)]"
            ...
            <AdminReveal className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <MetricCard accent="bg-[#128a93]" icon={Activity} label="Avg. utilisation" note={report.comparison} value={`${report.average}%`} />
            ...
            <AdminReveal className="grid gap-5 2xl:grid-cols-[1.25fr_0.75fr]">
                <article className={joinClasses(surface, "overflow-hidden p-5 text-itx-ink sm:p-6")}>
                    <div className="flex items-start justify-between">
                        ...
                        <span className="rounded-full bg-[#128a93]/12 px-3 py-1.5 text-[10px] font-bold text-[#0d6169]">This week</span>
                    </div>
                        ...
                        {report.utilisation.map((item) => (
                            <div className="flex h-full flex-col justify-end gap-2" key={item.day}>
                                <div className="flex flex-1 items-end rounded-t-xl bg-slate-100 shadow-inner">
                                    <div
                                        className="group relative w-full rounded-t-xl bg-gradient-to-t from-[#0d6169] to-[#17a870] shadow-[0_-8px_20px_-15px_rgba(18,138,147,0.5)] transition-colors"
                ...
                <article className={joinClasses(surface, "p-5 text-itx-ink sm:p-6")}>
                    ...
                    <div className="mt-7 rounded-2xl bg-slate-50 p-4">
                        <p className="text-[10px] font-extrabold uppercase tracking-[0.1em] text-slate-500">Demand insight</p>
