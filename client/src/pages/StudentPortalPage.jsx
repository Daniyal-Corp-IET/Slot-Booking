...
function StudentPortalContent() {
    ...
    if (!student) {
        return (
            <main className="portal-canvas flex min-h-screen items-center justify-center p-5 text-white">
                <section className="portal-surface w-full max-w-lg rounded-4xl border p-7 text-center sm:p-9">
                    <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#2f9db0]">Student portal</p>
                    <h1 className="mt-3 text-2xl font-semibold tracking-[-0.04em]">No student profile is available.</h1>
                    <p className="mt-3 text-sm leading-6 text-white/55">
                        {error || "Ask the lab administrator to create your account, then log in with the assigned student ID."}
                    </p>
                    <button className="mt-6 h-12 rounded-2xl bg-[#3ee7c2] px-6 text-sm font-bold text-[#082330]" onClick={handleSignOut} type="button">
                        Return to login
                    </button>
                </section>
            </main>
